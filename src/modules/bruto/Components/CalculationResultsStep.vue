  ...
  <div class="calculation-results-step">
    <div class="step-header">
      ...
      <p class="step-description">
        Revise los resultados del cálculo de bono sábados para cada colaborador
      </p>
    </div>
      ...
      <div v-if="hasResults && !isCalculating" class="results-section">
        ...
        <div class="summary-section">
          ...
          <div class="summary-cards">
            ...
            <div class="summary-card">
              <div class="summary-label">Total Bono Pactado</div>
              <div class="summary-value">{{ formatCurrency(totalBonoPactado) }}</div>
            </div>
            <div class="summary-card">
              <div class="summary-label">Total Diferencia Líquida</div>
              <div class="summary-value" :class="{ 'negative': totalDiferenciaLiquida < 0 }">
                {{ formatCurrency(totalDiferenciaLiquida) }}
              </div>
            </div>
            <div class="summary-card">
              <div class="summary-label">Total Diferencia Bruto</div>
              <div class="summary-value" :class="{ 'negative': totalDiferenciaBruto < 0 }">
                {{ formatCurrency(totalDiferenciaBruto) }}
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="results-list">
            ...
            >
              ...
              <div class="input-data">
                ...
                <div class="data-grid">
                  <div class="data-item">
                    <span class="data-label">Horas Extras:</span>
                    <span class="data-value">{{ result.horasExtrasTrabajadas }}h</span>
                  </div>
                  <div class="data-item">
                    <span class="data-label">Valor Hora Extra:</span>
                    <span class="data-value">{{ formatCurrency(result.valorHoraExtra) }}</span>
                  </div>
                  <div class="data-item">
                    <span class="data-label">Bono Pactado:</span>
              ...
              <div class="main-results">
                <h6 class="section-subtitle">Resultados</h6>
                <div class="results-grid">
                  <div class="result-item highlight">
                    <div class="result-label">Diferencia Líquida</div>
                    <div class="result-amount" :class="{ 'negative': result.result.diferenciaLiquida < 0 }">
                      {{ formatCurrency(result.result.diferenciaLiquida) }}
                    </div>
                  </div>
                  <div class="result-item highlight">
                    <div class="result-label">Diferencia Bruto</div>
                    <div class="result-amount" :class="{ 'negative': result.result.diferenciaBruto < 0 }">
              ...
              <div class="detailed-breakdown">
                <h6 class="section-subtitle">Desglose de Liquidación</h6>
                <div class="breakdown-formula">
                  <div class="formula-item">
                    <span class="formula-label">Bono Pactado</span>
                    <span class="formula-value">{{ formatCurrency(result.result.montoBonoPactado) }}</span>
                  </div>
                  <div class="formula-operator">−</div>
                  <div class="formula-item">
                    <span class="formula-label">Total Horas Extras</span>
                    <span class="formula-value">{{ formatCurrency(result.result.montoTotalHorasExtras) }}</span>
                  </div>
                  <div class="formula-operator">=</div>
                  <div class="formula-item result">
                    <span class="formula-label">Diferencia Líquida</span>
                    <span class="formula-value" :class="{ 'negative': result.result.diferenciaLiquida < 0 }">
                      {{ formatCurrency(result.result.diferenciaLiquida) }}
                    </span>
                  </div>
                </div>
...
const totalBonoPactado = computed(() => {
  return props.calculationResults.reduce((sum, result) => sum + result.montoBonoPactado, 0);
});

const totalDiferenciaLiquida = computed(() => {
  return props.calculationResults.reduce((sum, result) => {
    // Solo sumar si la diferencia líquida es positiva
    return result.result.diferenciaLiquida > 0 ? sum + result.result.diferenciaLiquida : sum;
  }, 0);
});

const totalDiferenciaBruto = computed(() => {
  return props.calculationResults.reduce((sum, result) => {
    // Solo sumar si la diferencia bruta es positiva
    return result.result.diferenciaBruto > 0 ? sum + result.result.diferenciaBruto : sum;
  }, 0);
});
...
const handleExport = async () => {
  ...
  try {
    ...
    if (!validateExportData(props.calculationResults)) {
      throw new Error('Los datos no tienen el formato correcto para exportación');
    }
    ...
    const fileName = `bono_sabados_${dateStr}_${timeStr}.xlsx`;

    // Exportar con resumen completo
    await exportBonoSabadosWithSummary(props.calculationResults, fileName);
